...
{
    public class RunMacro : CodedWorkflow
    {
        [Workflow]
        public void Execute()
        {
            ...
            // screen.Click(Descriptors.MyApp.FirstScreen.SettingsButton);
                                                 using (var pow = powerpoint.UsePowerPointPresentation("MAcroPPT.pptm"))
            {
               var macroArgList = new List<object> {true};
                pow.RunMacro("visibleTest",macroArgList);
